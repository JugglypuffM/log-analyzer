...
object GlobFileFinder {
  ...
  def getBaseDirAndPattern(path: String): Either[Throwable, (Path, PathMatcher)] =
    val normalizedInput = path.strip()
    val separatorIndex = normalizedInput.lastIndexOf("/")
    
    Either.catchNonFatal(
      if (separatorIndex == -1) (Path("."), fileSystem.getPathMatcher(s"glob:$normalizedInput"))
      else
        ...
        if (dirSkipIndex == -1) (Path(maybeBaseDir), fileSystem.getPathMatcher(s"glob:$maybePattern"))
        else
          val baseDir = Path(maybeBaseDir.substring(0 ,dirSkipIndex))
          val skippedDirs = maybeBaseDir.substring(dirSkipIndex)
          val pattern = fileSystem.getPathMatcher(s"glob:$skippedDirs$maybePattern")

          (baseDir, pattern)
    )

  def findFilesByGlob[F[_] : Files](baseDir: Path, globPattern: PathMatcher): Stream[F, Path] = {
    Files[F]
      .walk(baseDir)
